...
// CSV Report variables
var CSVseparation = ';';
//var CSVseparation = ',';
var CSVReportTxt = 'Full code submitted'+CSVseparation+'Check digit from code submitted'+CSVseparation+'Corrected check digit'+CSVseparation+'Corrected code'+'\n';
...
function checkEditedText(myGroup){
    ...
    var settingsCodeTF = getCODEtextField(myGroup);
    ...
    var wantedCode = settingsCodeTF.contents;
...
function launchEdit(){
    ...
    for (i=0;i<barcodeToEditList.length; i++){
        ...
        settingsCodeTF = getCODEtextField(myItem);
        myWantedCode = settingsCodeTF.contents;
        ...
        if (myCorrectedCode != myWantedCode){
            ...
            settingsCodeTF.contents = myCorrectedCode;
...
function drawEAN13(myItem){
    ...
    var myFillColor = settingsCodeTF.textRanges[0].characterAttributes.fillColor;
...
function writeHumanReadableCode(myCoriusBarcode){
    ...
    var settingsCodeTF = getCODEtextField(myCoriusBarcode);
...
function getCODEtextField(myGrp){
    var result;
    var i;
    var myItem;
    var codeFound = false;
    ...
    for (var i=0;i<myGrp.groupItems.getByName('Settings').textFrames.length && codeFound == false;i++){
        //myItem = myGrp.groupItems.getByName('Settings').textFrames.getByName('CODE_ITF14').contents;
        myItem = myGrp.groupItems.getByName('Settings').textFrames[i];
        myItemNameStart = myItem.name.substr(0,5);
        if (myItemNameStart == 'CODE_'){
            result  = myItem;
            codeFound = true;
        }
    }
...
function createReport(){
    ...
    for (i=0; i<correctedErrorList.length; i++){
        CSVReportTxt += correctedErrorList[i].join(CSVseparation);
        CSVReportTxt += '\n';
    }
...
function vectorizeAllTexts(myDoc){
    ...
    for (i=0;i<myDoc.layers.length;i++){
        ...
        if (mylayer.visible){ 
            ...
            for (k=mylayer.symbolItems.length;k>0;k--){
                myInstance = mylayer.symbolItems[k-1];
